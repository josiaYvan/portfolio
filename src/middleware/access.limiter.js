 ...
 * @copyright ©2024 ― Md. Samiur Rahman (Mukul). All rights reserved.
 * @version v0.0.1
 *
 */
...
const apiLimiter = rateLimit({
  windowMs: 24 * 60 * 60 * 1000, // 1 jour
  max: 20, // limit each IP to 10 login requests per `window` (here, per 1 day)
  message: { message: 'Too many login attempts from this IP, please try again after a 60 second pause' },
  handler: async (req, res, _next, options) => {
    logger.error('apiLimiter => middleware');
